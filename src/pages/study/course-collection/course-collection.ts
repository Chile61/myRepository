...
@Component({
    selector: 'course-collection-page',
    templateUrl: 'course-collection.html'
})

//我的课程收藏
export class CourseCollectionPage extends BasePage{

    @ViewChild('CourseCollectionContent') CourseCollectionContent;

    private REFRESH_CODE = 'CourseCollectionPage';//已读置灰刷新列表通知Code
    ...
    toTop(){
        this.CourseCollectionContent.scrollToTop();
    }

    //获取搜索课程列表
    getCourseCollection(refresher?: any) {
        let url = this.apiUrls.getUrlMyCourseCollection();
        let params = {
    ...
    doRefresh(refresher, isRefresh: boolean) {
        ...
        this.getCourseCollection(refresher);
    }
