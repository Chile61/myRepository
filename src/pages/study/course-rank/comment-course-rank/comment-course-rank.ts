...
@Component({
    ...
    templateUrl: 'comment-course-rank.html'
})

//课程评论排行
export class CourseCommentRankPage extends BasePage {

    @ViewChild('CourseRankContent') CourseRankContent;

    private REFRESH_CODE = 'CourseCommentRankPage';//已读置灰刷新列表通知Code

    public courseList: Array<CourseMiniVo> = [];//评论排行课程
    ...
    //获取评论排行课程列表
    getCommentCourseList(refresher?: any) {
        let params = {
        ...
        this.httpUtil.get({
            url: this.apiUrls.getUrlMostComment(), param: params, success: (res) => {
                if (this.pageNum == 1) {
                    this.courseList = [];
                }
    ...
    doRefresh(refresher, isRefresh: boolean) {
        ...
        this.getCommentCourseList(refresher);
    }
