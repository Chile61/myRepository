...
@Component({
    ...
    templateUrl: 'favorite-course-rank.html'
})

//课程收藏排行
export class CourseFavoriteRankPage extends BasePage {

    @ViewChild('CourseRankContent') CourseRankContent;

    private REFRESH_CODE = 'CourseFavoriteRankPage';//已读置灰刷新列表通知Code

    public courseList: Array<CourseMiniVo> = [];//收藏排行课程
    ...
    //获取点赞排行课程列表
    getFavoriteCourseList(refresher?: any) {
        let params = {
        ...
        this.httpUtil.get({
            url: this.apiUrls.getUrlMostFavorite(), param: params, success: (res) => {
                if (this.pageNum == 1) {
                    this.courseList = [];
                }
    ...
    doRefresh(refresher, isRefresh: boolean) {
        ...
        this.getFavoriteCourseList(refresher);
    }
